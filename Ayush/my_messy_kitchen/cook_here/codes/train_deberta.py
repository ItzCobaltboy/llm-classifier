"""
train_deberta.py — Grid-search training for DeBERTa-v3-base.
...
Note: DeBERTa-v3 uses a SentencePiece tokenizer; `token_type_ids` are not
returned by default, which is handled correctly by AutoModel.
"""
    ...
    for lr in LRS:
        for batch_size in BATCH_SIZES:
            for use_differential_lr in LR_MODES:
                ...
                run_name = f"deberta__maxlen{max_len}__lr{lr}__bs{batch_size}__{lr_tag}"
...
subprocess.run(["zip", "-j", "deberta_outputs.zip"] + glob.glob("/kaggle/working/*deberta*"))
print("\nAll DeBERTa runs complete. Outputs zipped → deberta_outputs.zip")
