...
for size in ["small", "medium", "original"]:
    for split in ["train", "val", "test"]:
        print(f"Processing truncate_upsample: {size} | {split}")
        ...
        df_final = upsample(remove_duplicates(truncate_only(df)))
        
        out_dir = f"./Dataset_cleaned/truncate_upsample/Dataset_{size}/{split}"
