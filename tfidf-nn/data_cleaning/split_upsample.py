...
for size in ["small", "medium", "original"]:
    for split in ["train", "val", "test"]:
        print(f"Processing split_upsample: {size} | {split}")
        ...
        df_final = upsample(remove_duplicates(split_texts(df)))
        
        out_dir = f"./Dataset_cleaned/split_upsample/Dataset_{size}/{split}"
